...
setwd("C:/Users/ADMIN/Desktop/CEU/DA3/assignment3/")

data_in   <- "C:/Users/ADMIN/Desktop/CEU/DA3/assignment3/data/clean/"
data_out  <- "data/clean/"
...
#import data
daily_agg <- read_csv("C:/Users/ADMIN/Desktop/CEU/DA3/assignment3/data/clean/outdoor_pools.csv") %>% 
...
###########################x
# Evaluate best models on holdout set - model 5 and model prophet --------------------------------------------
###########################x

newdataprophet <- make_future_dataframe(model_prophet, include_history = F,periods = 366)
newdataprophet
y_hat_prop = predict(model_prophet, newdataprophet)
y_hat_prop
tail(y_hat_prop[c('yhat')])

data_holdout <- data_holdout %>% 
  mutate(y_hat_5 = predict(reg5, newdata = .)) %>% 
  mutate(y_hat_prop = predict(model_prophet, newdataprophet)$yhat)

rmse_holdout_best <- RMSE(data_holdout$QUANTITY, data_holdout$y_hat_5)
rmse_holdout_best

rmse_holdout_prophet <- RMSE(data_holdout$QUANTITY, data_holdout$y_hat_prop)
rmse_holdout_prophet
...
## model prophet
rmse_monthly_prophet <- data_holdout %>% 
  mutate(month = factor(format(date,"%b"), 
                        levels= unique(format(sort(.$date),"%b")), 
                        ordered=TRUE)) %>% 
  group_by(month) %>% 
  summarise(
    RMSE = RMSE(QUANTITY, y_hat_prop),
    RMSE_norm= RMSE(QUANTITY, y_hat_prop)/mean(QUANTITY)
  ) 

g_predictions_rmse_prophet <- ggplot(rmse_monthly_prophet, aes(x = month, y = RMSE_norm)) +
  geom_col(bg=color[2], color=color[2]) +
  labs( x = "Date (month)", y="RMSE (normalized by monthly sales)" ) +
  theme_bg() 
g_predictions_rmse_prophet
...
g_predictions_m <- ggplot(data=data_holdout %>% filter(month==8), aes(x=date, y=QUANTITY)) +
  ...
  scale_y_continuous(expand = c(0.01,0.01), limits = c(-50,650))+
