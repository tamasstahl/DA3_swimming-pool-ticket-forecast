...
data <- data %>%
  filter(Location %in% c('AQEI01', 'AQEJ01', 'AQMP01','AQRG01','AQSP01', 'AQSV01', 'AQWP01')) %>% #all outdoor pools
  filter(Category %in% c("ADMISTIER1","ADMISTIER2", "ADMISTIER2")) %>%
...
# Save workfile
write.csv(daily_agg,paste(data_out,"outdoor_pools.csv",sep=""), row.names = FALSE)               
